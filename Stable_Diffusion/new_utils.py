...
def text2image_ldm_stable(
    ...
    # 解析提示词，提取包含指定类别的单词，并记录这些单词的索引
    category_words = ["dog", "cat", "car","chair"]  # 指定类别
    word_indices = []
    for i, p in enumerate(prompt):
        indices = [j for j, word in enumerate(p.split()) if word in category_words]
        word_indices.append(indices)
    
    # 将单词索引传递给控制器
    print(word_indices)
    controller.set_word_indices(word_indices)
