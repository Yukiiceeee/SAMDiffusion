...
# 定义输入和输出目录
input_dir = '/data2/mxy/SAMDiffusion/DiffMask_VOC/VOC_Multi_Attention_cat_sub_1000_NoClipRetrieval_sample/sam_output'
output_dir = '/data2/mxy/SAMDiffusion/DiffMask_VOC/VOC_Multi_Attention_cat_sub_1000_NoClipRetrieval_sample/repair_output'
...
for filename in os.listdir(input_dir):
    if filename.endswith('.png'):
        ...
        mask = gray_image < 0.4  # 假设蓝色区域为需要修复的部分

        # 使用线性插值修复图像
        repaired_image = gray_image.copy()

        # 定义邻域的半径
        radius = 2  # 2 对应 5x5 的邻域
        ...
        for y, x in zip(*missing_pixels):
            ...
            if np.any(~neighborhood_mask):  # 只在存在有效邻域值时才进行插值
                repaired_image[y, x] = 0.5010905882352942
